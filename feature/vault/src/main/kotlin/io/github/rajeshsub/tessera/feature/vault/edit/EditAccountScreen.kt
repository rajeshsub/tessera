...
fun EditAccountScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.effects.collect { effect ->
            when (effect) {
                is EditAccountViewModel.UiEffect.Saved -> onBack()
                is EditAccountViewModel.UiEffect.Deleted -> onBack()
                is EditAccountViewModel.UiEffect.Error -> snackbar.showSnackbar(effect.message)
            }
        }
    }
        ...
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                OtpType.entries.forEach { type ->
                    RadioButton(selected = form.type == type, onClick = { viewModel.updateType(type) })
                    Text(type.name)
                    Spacer(Modifier.padding(end = 16.dp))
                }
            }
            Spacer(Modifier.height(8.dp))
            EditAlgorithmDropdown(selected = form.algorithm, onSelect = viewModel::updateAlgorithm)
            Spacer(Modifier.height(8.dp))
            OutlinedTextField(
